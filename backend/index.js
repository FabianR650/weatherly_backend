...
function buildDailyForecast(list) {
    ...
    const server = http.createServer(async (req, res) => {
        ...
        if(path === "/api/weather") {
            ...
            if (!city) {
                return sendJSON(res, 404, {error: "City is required"})
            }
            ...
            try {
                ...
                console.log("Weather city error:", err.response?.data || err.message);
        ...
        if(path === "/api/weather/coords") {
            ...
            if(!lat || !lon) {
                return sendJSON(res, 404, {error: "lat and lon required" });
            }
            try {
                const [currentRes, forecastRes] = await Promise.all ([
                    axios.get(
                ...
                console.log("Weather coords error:", err.response?.data || err.message);
                return sendJSON(res, 500, {error: "Weather lookup failed" });
            }
        }
